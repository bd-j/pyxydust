...
class DraineLi(SpecLibrary):
    ...
    def generate_spectrum(self, umin, umax, gamma, qpah, alpha, mdust = 1.0):
        """
        Returns the IR SED of a model(s) given the model parameters.
        ...
        check = [umin.shape[0] == umax.shape[0],umin.shape[0] == qpah.shape[0]]
        if False in check:
    ...
    def library_from_dustEM():
        """
        If python bindings to DustEM ever happen, you could call it
        from here to fill the model recarray with a denser grid than
        Draine provides.
        """
        pass
